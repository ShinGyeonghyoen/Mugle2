...
st.markdown("""
    ...
    /* 파칭코 번쩍번쩍 애니메이션 */
    @keyframes feverTime {
        0% { border-color: #0A84FF; box-shadow: 0 0 10px #0A84FF; }
        25% { border-color: #FFD700; box-shadow: 0 0 20px #FFD700; }
        50% { border-color: #FF453A; box-shadow: 0 0 10px #FF453A; }
        75% { border-color: #32CD32; box-shadow: 0 0 20px #32CD32; }
        100% { border-color: #0A84FF; box-shadow: 0 0 10px #0A84FF; }
    }
    ...
    .fever-window {
        ...
        margin-bottom: 15px;
        animation: feverTime 0.3s infinite; /* 더 빠르게 번쩍임 */
    }
    ...
    .dance-img { width: 45%; border-radius: 10px; }
...
menu_pool = {
    ...
    "👨‍🍳 백종원": ["홍콩 반점", "새마을 식당", "조보아씨 내려와 봐유", "빽다방", "돌아갈까봐 그래유"]"돌아갈까봐 그래유"]
}
...
if st.session_state.app_state == "READY":
    if st.session_state.final_choice:
        ...
        st.markdown(f'''
            <div class="fever-window">
                ...
                <div class="menu-display">
                    ...
                    <div class="menu-text" style="font-size:14px; color:#32CD32;">🕺 박 대리와 김 사원의 기쁨의 댄스 💃</div>
                    <div style="display: flex; justify-content: center; gap: 10px; padding-top: 10px;">
                        <div style="font-size: 40px;">🏃‍♂️💨</div><div style="font-size: 40px;">💃✨</div>
                    </div>
                    <p style="font-size:12px; color:#8E8E93;">(Tell Me & 다시 만난 세계 동시 상영 중)</p>
                </div>
            </div>
        ''', unsafe_allow_html=True)
        ...
        st.markdown('<div class="ios-window"><div class="menu-display"><div class="menu-text" style="color:#8E8E93 !important;">📂 결재 서류를 대기 중...</div></div></div>', unsafe_allow_html=True)
